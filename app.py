...
[data-testid="stSidebar"]{
    background:linear-gradient(180deg,#0a1628,#0d1f3c) !important;
...
[data-testid="collapsedControl"] {
    ...
    box-shadow:0 0 18px rgba(59,130,246,.9) !important;
}
...
[data-testid="stSidebar"] [data-testid="stDownloadButton"] button:hover{
    background:linear-gradient(135deg,#1d4ed8,#2563eb) !important;
    border:1px solid #60a5fa !important;color:#fff !important;
...
[data-testid="metric-container"]{
    ...
    border:1px solid #1e3a5f !important;border-radius:12px !important;padding:12px 16px !important;
}
[data-testid="metric-container"] label{color:#64a0d4 !important;font-size:11px !important;font-weight:700 !important;text-transform:uppercase;letter-spacing:.5px}
[data-testid="metric-container"] [data-testid="stMetricValue"]{color:#f0f9ff !important;font-size:24px !important;font-weight:900 !important}

/* ── Expander ── */
[data-testid="stExpander"]{background:#0a1628 !important;border:1px solid #1e3a5f !important;border-radius:10px !important}
...
.stButton>button:hover{box-shadow:0 6px 20px rgba(59,130,246,.6) !important;transform:translateY(-1px) !important}

/* ── Selectbox ── */
.stSelectbox>div>div{background:#0f2744 !important;border:1px solid #3b82f6 !important;color:#e2e8f0 !important;border-radius:8px !important}

/* ── File uploader ── */
[data-testid="stFileUploader"]{background:#0a1e3c !important;border:1px dashed #3b82f6 !important;border-radius:10px !important}
...
.hero-banner::before{content:'';position:absolute;top:-60px;right:-60px;width:220px;height:220px;background:radial-gradient(circle,rgba(59,130,246,.15),transparent 70%);border-radius:50%}
.hero-banner h1{margin:0 0 6px;font-size:26px;font-weight:900;background:linear-gradient(90deg,#60a5fa,#a78bfa,#34d399);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hero-banner p{margin:0;color:#64748b;font-size:13px}
.hero-pills{display:flex;gap:8px;margin-top:12px;flex-wrap:wrap}
.pill{background:rgba(59,130,246,.12);border:1px solid rgba(59,130,246,.3);border-radius:20px;padding:3px 12px;font-size:11px;color:#93c5fd;font-weight:600}

/* ── Module banner ── */
.module-banner{border-radius:14px;padding:18px 24px;margin-bottom:22px;position:relative;overflow:hidden}
.module-banner::after{content:'';position:absolute;inset:0;background:rgba(0,0,0,.35);border-radius:14px;pointer-events:none}
...
.problem-tag{display:inline-block;margin-top:8px;background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.15);border-radius:20px;padding:3px 12px;font-size:11px;color:rgba(255,255,255,.8)}

/* ── AI box ── */
.ai-box{background:linear-gradient(135deg,#07101f,#0c1a30,#091525);border:1px solid #1e4070;border-radius:14px;padding:20px 24px;margin-top:18px;box-shadow:0 8px 32px rgba(0,0,0,.5)}
.ai-badge{display:inline-block;background:linear-gradient(135deg,#4f46e5,#7c3aed);border-radius:6px;padding:3px 10px;font-size:10px;font-weight:800;color:#fff;letter-spacing:1px;text-transform:uppercase;margin-bottom:10px}
.ai-model{font-size:11px;color:#475569;margin-left:8px}
.ai-hr{border:none;border-top:1px solid #1e3a5f;margin:10px 0 14px}
.ai-text{color:#cbd5e1;font-size:13.5px;line-height:1.85;white-space:pre-wrap}
.ai-text strong{color:#93c5fd;font-weight:700}

/* ── Section headers ── */
.sh{display:flex;align-items:center;gap:10px;margin:20px 0 12px;padding-bottom:8px;border-bottom:1px solid #1e3a5f}
.sh span{font-size:13px;font-weight:700;color:#93c5fd;text-transform:uppercase;letter-spacing:.5px}

/* ── Alerts ── */
.alert-r{background:#1a0a0a;border-left:4px solid #ef4444;border-radius:8px;padding:10px 14px;color:#fca5a5;font-size:12px;margin-bottom:10px}
.alert-y{background:#1a140a;border-left:4px solid #f59e0b;border-radius:8px;padding:10px 14px;color:#fcd34d;font-size:12px;margin-bottom:10px}
.alert-b{background:#0a1a2e;border-left:4px solid #3b82f6;border-radius:8px;padding:10px 14px;color:#93c5fd;font-size:12px;margin-bottom:10px}
</style>
...
_DB = dict(
    ...
    xaxis=dict(gridcolor="#0f2240",linecolor="#1e3a5f",tickfont=dict(color="#64748b"),zerolinecolor="#1e3a5f"),
    yaxis=dict(gridcolor="#0f2240",linecolor="#1e3a5f",tickfont=dict(color="#64748b"),zerolinecolor="#1e3a5f"),
)
_LEG = dict(bgcolor="rgba(10,22,40,.8)",bordercolor="#1e3a5f",borderwidth=1,
...
PAL=["#6366f1","#10b981","#f59e0b","#ec4899","#38bdf8","#8b5cf6","#fb923c","#84cc16","#06b6d4","#f43f5e"]
...
META = {
    "forecast":  {"icon":"📈","title":"Demand Forecast Engine",  "color":"#6366f1","grad":"linear-gradient(135deg,#312e81,#1e1b4b)","problem":"Analysts manually tweak spreadsheets - 30pct forecast errors - overstock or stockouts bleed cash every quarter."},
    "inventory": {"icon":"📦","title":"Inventory Health Monitor","color":"#10b981","grad":"linear-gradient(135deg,#064e3b,#065f46)","problem":"No dynamic EOQ/safety-stock model - working capital locked in dead stock OR stockouts tank fill rate."},
    "supplier":  {"icon":"🏭","title":"Supplier Risk Radar",     "color":"#f59e0b","grad":"linear-gradient(135deg,#78350f,#92400e)","problem":"Single-source concentration + zero early-warning scores - one supplier failure cascades into a production shutdown."},
    "sop":       {"icon":"🔄","title":"S&OP Alignment Monitor",  "color":"#ec4899","grad":"linear-gradient(135deg,#831843,#9d174d)","problem":"Sales, Demand Planning, Production and Procurement run different numbers - wrong product, wrong time, wrong place."},
    "kpi":       {"icon":"📊","title":"KPI Command Center",      "color":"#38bdf8","grad":"linear-gradient(135deg,#0c4a6e,#075985)","problem":"OTIF, fill rates, lead times tracked in 10 spreadsheets by 10 people - leadership decides on stale conflicting data."},
}
...
def banner(k):
    m=META[k]
    st.markdown(
        f'<div class="module-banner" style="background:{m["grad"]};border:1px solid {m["color"]}40">'
        f'<div class="module-banner-inner"><h3>{m["icon"]} {m["title"]}</h3>'
        f'<p>Problem: {m["problem"]}</p>'
        f'<span class="problem-tag">One of 5 core supply chain resource problems solved here</span>'
        f'</div></div>', unsafe_allow_html=True)

def sh(icon,label):
    st.markdown(f'<div class="sh"><span style="font-size:16px">{icon}</span><span>{label}</span></div>',unsafe_allow_html=True)

def ai_box(text):
    text=re.sub(r'\*\*(.+?)\*\*','<strong>\\1</strong>',text)
    st.markdown(
        f'<div class="ai-box"><span class="ai-badge">AI Insight</span>'
        f'<span class="ai-model">llama-3.3-70b-versatile via Groq</span>'
        f'<hr class="ai-hr"><div class="ai-text">{text}</div></div>',
        unsafe_allow_html=True)
...
def mod_forecast(api_key):
    ...
    fcast=[max(0,round(ic+sl*i+np.random.uniform(-.04,.04)*(ic+sl*i))) for i in range(n,len(months))]
    ...
    fig.add_trace(go.Bar(x=months[:n],y=act,name="Actual",
        marker=dict(color=act,colorscale=[[0,"#312e81"],[.5,"#6366f1"],[1,"#a5b4fc"]],showscale=False,line=dict(width=0))))
    fig.add_trace(go.Scatter(x=months[:n],y=[round(v) for v in ema],name="EMA",mode="lines+markers",
    ...
    pc(dark(fig2,h=220,leg=False,title="MoM Growth %",margin=dict(t=36,b=16,l=8,r=8)))

    if promo: alrt("b",f"<b>Promo months detected:</b> {', '.join(promo)} -- factored into projection")
    ...
    if st.button("Run AI Forecast Analysis",key="btn_f"):
        with st.spinner("Analyzing with Groq..."):
            ...
            ai_box(groq_insight(api_key,
                ...
                "EMA projection: " + _safe_str(dict(zip(months[n:], fcast))) + "\n"
                "Forecast accuracy: " + str(acc) + "% (target 90%)\n"
                "Demand volatility: +-" + str(vol) + "%\n"
                "Promo months: " + _safe_str(promo) + "\n\n"
                "Provide:\n"
...
def mod_inventory(api_key):
    ...
    locked=float(over.locked_capital.sum())
    twc=float(df.working_capital.sum())
    ...
    fig=go.Figure()
    fig.add_trace(go.Bar(x=nm,y=df.stock.tolist(),        name="Stock",        marker_color="#6366f1"))
    fig.add_trace(go.Bar(x=nm,y=df.reorder_point.tolist(),name="Reorder Point",marker_color="#f59e0b"))
    fig.add_trace(go.Bar(x=nm,y=df.safety_stock.tolist(), name="Safety Stock", marker_color="#ef4444"))
    fig.add_trace(go.Scatter(x=nm,y=df.eoq.tolist(),name="EOQ",mode="markers+lines",
        marker=dict(size=10,color="#10b981",symbol="diamond"),line=dict(dash="dot",color="#10b981",width=2)))
    pc(dark(fig,h=340,barmode="group"))

    sh("💰","Capital & Days of Supply")
    ca,cb=st.columns(2)
    with ca:
        f2=go.Figure(go.Pie(labels=nm,values=df.working_capital.tolist(),hole=0.52,
    ...
    with cb:
        ...
        f3=go.Figure(go.Bar(x=nm,y=dos,
    ...
    if st.button("Run AI Inventory Analysis",key="btn_inv"):
        with st.spinner("Analyzing with Groq..."):
            rows=[{"sku":str(r.sku),"item":str(r.item_name),"stock":int(r.stock),
                   "reorder":int(r.reorder_point),"eoq":int(r.eoq),
                   "safety_stock":int(r.safety_stock),"daily_demand":int(r.daily_demand),
                   "lead_time":int(r.lead_time_days),"days_supply":float(r.days_of_supply),
                   "status":str(r.status),"wc":float(r.working_capital),
                   "locked":float(r.locked_capital)} for _,r in df.iterrows()]
            ai_box(groq_insight(api_key,
                "You are a senior inventory optimization analyst. Be specific with numbers. Use **bold** for key findings.",
                "Inventory data: " + _safe_str(rows) + "\n"
                "Total working capital: $" + f"{twc:,.0f}" + "\n"
                "Locked in overstock: $" + f"{locked:,.0f}" + "\n\n"
                "Provide:\n"
...
def mod_supplier(api_key):
    ...
    tot=float(df.annual_spend.sum())
    ar =float(df[df.risk_tier.isin(["High","Critical"])].annual_spend.sum())
    ss =float(df[df.single_source==1].annual_spend.sum()) if "single_source" in df.columns else 0.0
    ...
    sn=df.supplier_name.tolist()
    fig=go.Figure()
    fig.add_trace(go.Bar(x=sn,y=df.delivery_pct.tolist(),name="Delivery %", marker_color="#6366f1"))
    fig.add_trace(go.Bar(x=sn,y=df.quality_pct.tolist(), name="Quality %",  marker_color="#10b981"))
    fig.add_trace(go.Bar(x=sn,y=df.cost_score.tolist(),  name="Cost Score", marker_color="#f59e0b"))
    fig.add_trace(go.Scatter(x=sn,y=df.composite_score.tolist(),name="Composite",
    ...
    fig.add_hline(y=70,line_dash="dot",line_color="#ef4444",annotation_text="Risk (70)",annotation_font_color="#ef4444")
    fig.add_hline(y=85,line_dash="dot",line_color="#10b981",annotation_text="Healthy (85)",annotation_font_color="#10b981")
    pc(dark(fig,h=340,barmode="group"))
    ...
    with ca:
        ...
        for _,row in df.iterrows():
            ...
            f2.add_trace(go.Scatter(x=[float(row.composite_score)],y=[float(row.annual_spend)],
                mode="markers+text",text=[row.supplier_name],textposition="top center",
                textfont=dict(color="#e2e8f0",size=10),
                marker=dict(size=float(row.lead_time_days)*3.5,color=clr,opacity=.75,
    ...
    with cb:
        f3=go.Figure(go.Pie(labels=df.supplier_name.tolist(),values=df.annual_spend.tolist(),hole=0.55,
    ...
    cats=["Delivery","Quality","Cost","Lead Time Inv.","Composite"]
    vals=[float(row.delivery_pct),float(row.quality_pct),float(row.cost_score),
          max(0.0,100.0-(float(row.lead_time_days)/30)*100),float(row.composite_score)]
    ...
    if st.button("Run AI Supplier Analysis",key="btn_sup"):
        with st.spinner("Analyzing with Groq..."):
            rows=[{"name":str(r.supplier_name),"delivery":int(r.delivery_pct),
                   "quality":int(r.quality_pct),"cost":int(r.cost_score),
                   "lead_days":int(r.lead_time_days),"spend":int(r.annual_spend),
                   "score":float(r.composite_score),"risk":str(r.risk_tier),
                   "spend_share":float(r.spend_share)} for _,r in df.iterrows()]
            ss_names=[str(r.supplier_name) for _,r in df.iterrows()
                      if "single_source" in df.columns and r.single_source==1]
            ai_box(groq_insight(api_key,
                "You are a procurement and supplier risk expert. Be specific with numbers. Use **bold** for key findings.",
                "Supplier portfolio: " + _safe_str(rows) + "\n"
                "Single-source suppliers: " + _safe_str(ss_names) + "\n"
                "Total spend: $" + f"{tot:,.0f}" + " | At-risk: $" + f"{ar:,.0f}" + "\n\n"
                "Provide:\n"
...
def mod_sop(api_key):
    ...
    gdf=pd.DataFrame(gd); wq=gdf.loc[gdf.gap_pct.idxmax()]; avg=float(gdf.gap_pct.mean())

    c1,c2,c3,c4=st.columns(4)
    c1.metric("Worst Quarter",      str(wq.quarter))
    ...
    if avg>10:  alrt("r",f"<b>Critical misalignment:</b> {avg:.1f}% avg gap -- HIGH revenue leakage")
    ...
    fig=go.Figure()
    for _,row in df.iterrows():
        fig.add_trace(go.Bar(name=str(row.department),x=[ql[q] for q in qs],
            y=[float(row[q]) for q in qs],marker_color=DC.get(row.department,"#64748b")))
    pc(dark(fig,h=340,barmode="group"))

    sh("🌡","Misalignment Heatmap")
    ca,cb=st.columns(2)
    with ca:
        f2=go.Figure(go.Bar(x=gdf.quarter.tolist(),y=gdf.gap.tolist(),
            marker=dict(color=gdf.gap_pct.tolist(),
                        colorscale=[[0,"#064e3b"],[.4,"#f59e0b"],[.7,"#ef4444"],[1,"#dc2626"]],
                        showscale=True,
                        colorbar=dict(title=dict(text="Gap %",font=dict(color="#94a3b8")),
                                      tickfont=dict(color="#94a3b8")),
                        line=dict(width=0)),
            text=gdf.gap_pct.apply(lambda v:f"{v:.1f}%").tolist(),
            textposition="outside",textfont=dict(color="#e2e8f0")))
        pc(dark(f2,h=280,leg=False,title="Unit Gap by Quarter",margin=dict(t=36,b=16,l=8,r=8)))
    with cb:
        hr2=[]
        for _,row in df.iterrows():
            r2={"Dept":str(row.department)}
            for q in qs: r2[ql[q]]=round((float(df[q].max())-float(row[q]))/float(df[q].max())*100,1)
            hr2.append(r2)
        hdf=pd.DataFrame(hr2).set_index("Dept")
        f3=go.Figure(go.Heatmap(z=hdf.values.tolist(),x=hdf.columns.tolist(),y=hdf.index.tolist(),
    ...
    if st.button("Run AI S&OP Analysis",key="btn_sop"):
        with st.spinner("Analyzing with Groq..."):
            dept_data=[{"dept":str(r.department),
                        "Q1":int(r.q1),"Q2":int(r.q2),"Q3":int(r.q3),"Q4":int(r.q4)}
                       for _,r in df.iterrows()]
            gap_data=[{"quarter":str(r.quarter),"gap":float(r.gap),"gap_pct":round(float(r.gap_pct),1)}
                      for _,r in gdf.iterrows()]
            ai_box(groq_insight(api_key,
                "You are a senior S&OP expert. Be precise with numbers. Use **bold** for key findings.",
                "Department signals (units): " + _safe_str(dept_data) + "\n"
                "Gap analysis: " + _safe_str(gap_data) + "\n"
                "ASP=$45/unit | Avg gap: " + f"{avg:.1f}%" + "\n\n"
                "Provide:\n"
...
def mod_kpi(api_key):
    ...
    for i,(_,row) in enumerate(df.iterrows()):
        ...
        cols[i].metric(str(row.kpi_name),f"{row.current_value}{row.unit}",
    ...
    for i,(_,row) in enumerate(df.iterrows()):
        ...
        tv=float(row.target_value); cv=float(row.current_value)
        fig=go.Figure(go.Indicator(
            mode="gauge+number+delta",value=cv,
            title={"text":str(row.kpi_name),"font":{"size":12,"color":"#94a3b8"}},
            delta={"reference":tv,"suffix":str(row.unit),
                   "increasing":{"color":"#10b981"},"decreasing":{"color":"#ef4444"}},
            number={"suffix":str(row.unit),"font":{"color":"#e2e8f0","size":22}},
            gauge={"axis":{"range":[0,tv*1.25],"tickfont":{"color":"#64748b","size":9}},
                   "bar":{"color":clr,"thickness":.28},
                   "bgcolor":"#0a1628","bordercolor":"#1e3a5f",
                   "threshold":{"line":{"color":"#fff","width":3},"thickness":.75,"value":tv},
                   "steps":[{"range":[0,tv*.7],"color":"#1a0a0a"},
                             {"range":[tv*.7,tv*.9],"color":"#1a140a"},
                             {"range":[tv*.9,tv*1.25],"color":"#061a10"}]}))
    ...
    ca,cb=st.columns(2)
    with ca:
        pct_list=df.pct.tolist()
        bc=["#10b981" if p>=100 else"#f59e0b" if p>=85 else"#ef4444" for p in pct_list]
        f2=go.Figure(go.Bar(x=pct_list,y=df.kpi_name.tolist(),orientation="h",marker_color=bc,
            text=[f"{p:.1f}%" for p in pct_list],textposition="outside",textfont=dict(color="#e2e8f0")))
    ...
    with cb:
        f3=go.Figure(go.Heatmap(z=[df.pct.tolist()],x=df.kpi_name.tolist(),y=["Performance"],
    ...
    f4=go.Figure()
    for _,row in df.iterrows():
        nl=row.kpi_name=="Avg Lead Time"; cv=float(row.current_value); tr=float(row.trend)
        clr="#10b981" if ((tr<=0) if nl else (tr>=0)) else "#ef4444"
        f4.add_trace(go.Scatter(
            x=["3mo ago","2mo ago","1mo ago","Now"],
            y=[cv-tr*3,cv-tr*2,cv-tr,cv],
            name=str(row.kpi_name),mode="lines+markers",
    ...
    if st.button("Generate AI KPI Brief",key="btn_kpi"):
        with st.spinner("Analyzing with Groq..."):
            rows=[{"kpi":str(r.kpi_name),"current":float(r.current_value),
                   "target":float(r.target_value),"unit":str(r.unit),
                   "trend":float(r.trend),"pct":float(r.pct),"gap":float(r.gap)}
                  for _,r in df.iterrows()]
            off=[str(r.kpi_name) for _,r in df.iterrows() if r.pct<100]
            ai_box(groq_insight(api_key,
                "You are a supply chain VP. Be direct and data-driven. Use **bold** for key findings.",
                "KPI data: " + _safe_str(rows) + "\n"
                "Off-target KPIs (" + str(len(off)) + " of " + str(len(df)) + "): " + _safe_str(off) + "\n"
...
def main():
    ...
    sel=st.sidebar.radio("Select Module",list(labels.keys()),label_visibility="collapsed")
    active=labels[sel]
    data_panel(active)
